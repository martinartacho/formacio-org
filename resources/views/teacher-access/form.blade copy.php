...
{{--  <p class="text-red-600 text-sm"> {{ $token->token ?? 'token null' }}</p>
 ...
 <p class="text-blue-600 text-sm"> Course:  {{ $course->id }} {{ $course  ?? 'Course null'}}</p> --}}
    ...
    @if(isset($purpose) && $purpose === 'consent')
     {{-- <h2 class="text-xl font-semibold mb-4">{{ __('Consentiment de tractament de les teves dades - RGPD') }}</h2> --}}
     <h2 class="text-xl font-semibold mb-4">Consentiment de tractament de les teves dades - RGPD</h2>
    ...
    @if($token->metadata && isset($token->metadata['payment_data_completed']))
         <h2 class="text-xl font-semibold mb-4">{{ __('Confirmació de dades econòmiques') }}</h2>
            <div class="mb-8 p-4 bg-green-50 border border-green-200 rounded-lg">
                <div class="flex items-center">
                    <svg class="w-5 h-5 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
