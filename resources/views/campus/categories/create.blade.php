{{-- resources/views/campus/categories/create.blade.php --}}
@extends('campus.shared.layout')

@section('title', __('campus.new_category'))
@section('subtitle', __('campus.new_category'))
...
@section('breadcrumbs')
    ...
    <li aria-current="page">
        <div class="flex items-center">
            <i class="bi bi-chevron-right text-gray-400 mx-1"></i>
            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">{{ __('campus.new_category') }}</span>
        </div>
    </li>
@endsection

@section('content')
    <form action="{{ route('campus.categories.store') }}" method="POST">
        @csrf
                ...
                <div id="category-preview" class="p-4 rounded-lg border border-gray-300">
                    <div class="flex items-center space-x-3">
                        <div class="h-12 w-12 bg-blue-100 rounded-lg flex items-center justify-center">
                            <i class="bi bi-tag text-blue-600 text-xl" id="preview-icon"></i>
                        </div>
                        <div>
                            <div class="font-medium text-gray-900" id="preview-name">Nom de la categoria</div>
                            <div class="text-sm text-gray-500" id="preview-description">Descripció de la categoria</div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updatePreview() {
            ...
            const description = document.getElementById('description').value || 'Descripció de la categoria';
